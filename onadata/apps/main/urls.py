...
urlpatterns += [
    ...
    re_path(r"^api/$", RedirectView.as_view(url=settings.STATIC_DOC, permanent=True)),
    ...
    re_path(
        r"^(?P<username>[^/]+)/forms/(?P<id_string>[^/]+)/stats$",
        viewer_views.charts,
        name="form-stats",
    ),
    ...
    re_path(r"^typeahead_usernames", main_views.username_list, name="username_list"),
    re_path(r"^(?P<username>[^/]+)/$", main_views.profile, name="user_profile"),
    re_path(
        r"^(?P<username>[^/]+)/profile$",
        main_views.public_profile,
        name="public_profile",
    ),
    re_path(
        r"^(?P<username>[^/]+)/settings",
        main_views.profile_settings,
        name="profile-settings",
    ),
    re_path(
        r"^(?P<username>[^/]+)/cloneform$",
        main_views.clone_xlsform,
        name="clone-xlsform",
    ),
    re_path(r"^(?P<username>[^/]+)/activity$", main_views.activity, name="activity"),
    re_path(
        r"^(?P<username>[^/]+)/activity/api$",
        main_views.activity_api,
        name="activity-api",
    ),
    re_path(r"^activity/fields$", main_views.activity_fields, name="activity-fields"),
    re_path(r"^(?P<username>[^/]+)/api-token$", main_views.api_token, name="api-token"),
    # form specific
    re_path(
        r"^(?P<username>[^/]+)/forms/(?P<id_string>[^/]+)$",
        main_views.show,
        name="form-show",
    ),
    re_path(
        r"^(?P<username>[^/]+)/forms/(?P<id_string>[^/]+)/qrcode$",
        main_views.qrcode,
        name="get_qrcode",
    ),
    re_path(
        r"^(?P<username>[^/]+)/forms/(?P<id_string>[^/]+)/api$",
        main_views.api,
        name="mongo_view_api",
    ),
    re_path(
        r"^(?P<username>[^/]+)/forms/(?P<id_string>[^/]+)/public_api$",
        main_views.public_api,
        name="public_api",
    ),
    re_path(
        r"^(?P<username>[^/]+)/forms/(?P<id_string>[^/]+)/delete_data$",
        main_views.delete_data,
        name="delete_data",
    ),
    re_path(
        r"^(?P<username>[^/]+)/forms/(?P<id_string>[^/]+)/edit$",
        main_views.edit,
        name="xform-edit",
    ),
    re_path(
        r"^(?P<username>[^/]+)/forms/(?P<id_string>[^/]+)/perms$",
        main_views.set_perm,
        name="set-xform-permissions",
    ),
    re_path(
        r"^(?P<username>[^/]+)/forms/(?P<id_string>[^/]+)/photos",
        main_views.form_photos,
        name="form-photos",
    ),
    re_path(
        r"^(?P<username>[^/]+)/forms/(?P<id_string>[^/]+)/doc/(?P<data_id>\d+)",  # noqa
        main_views.download_metadata,
        name="download-metadata",
    ),
    re_path(
        r"^(?P<username>[^/]+)/forms/(?P<id_string>[^/]+)/delete-doc/(?P<data_id>\d+)",
        main_views.delete_metadata,  # noqa
    ...
    re_path(
        r"^(?P<username>[^/]+)/forms/(?P<id_string>[^/]+)/addservice$",
        restservice_views.add_service,
        name="add_restservice",
    ),
    re_path(
        r"^(?P<username>[^/]+)/forms/(?P<id_string>[^/]+)/delservice$",
        restservice_views.delete_service,
        name="delete_restservice",
    ),
    re_path(
        r"^(?P<username>[^/]+)/forms/(?P<id_string>[^/]+)/update$",
        main_views.update_xform,
        name="update-xform",
    ),
    re_path(
        r"^(?P<username>[^/]+)/forms/(?P<id_string>[^/]+)/preview$",
    ...
    # xform versions urls
    re_path(
        r"^api/v1/forms/(?P<pk>[^/.]+)/versions/(?P<version_id>[^/.]+)$",  # noqa
